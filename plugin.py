...
#           4.2.0:  remove checkTemp for RM2 device: can made trouble in some case and not really make sense.
#
...
#
"""
<plugin key="BroadlinkRM2" name="Broadlink RM2 with Kodi Remote" author="zak45" version="4.2.0" wikilink="http://www.domoticz.com/wiki/plugins/BroadlinkRM2.html" externallink="https://github.com/mjg59/python-broadlink">
    <params>
...
def onStart():
    ...
    if (Parameters["Mode3"] == 'DIS'):
        ...
        if (1 not in Devices):
            ...
            Domoticz.Device(Name="Command", Unit=1, TypeName="Selector Switch", Switchtype=18, Image=12,
        ...
        if (255 not in Devices and Parameters["Mode4"] == 'yes'):
            ...
            Domoticz.Device(Name="Import", Unit=255, TypeName="Selector Switch", Switchtype=18, Image=12,
                            Options=Options, Used=1).Create()
        ...
        if (1 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Status', Unit=1, Type=17, Image=17, Switchtype=17,
                            Used=1).Create()
        if (2 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Temperature', Unit=2, TypeName="Temperature",
                            Used=1).Create()
        if (3 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Humidity', Unit=3, TypeName="Humidity", Used=1).Create()
        if (4 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Air Quality', Unit=4, TypeName="Air Quality",
                            Used=1).Create()
        if (5 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Noise', Unit=5, TypeName="Sound Level", Used=1).Create()
        if (6 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Light', Unit=6, TypeName="Illumination",
                            Used=1).Create()

    elif (Parameters["Mode3"] == 'MP1'):
        if (1 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - Status', Unit=1, TypeName="Switch", Image=9,
                            Used=1).Create()
        if (2 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - plug1', Unit=2, TypeName="Switch", Image=1,
                            Used=1).Create()
        if (3 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - plug2', Unit=3, TypeName="Switch", Image=1,
                            Used=1).Create()
        if (4 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - plug3', Unit=4, TypeName="Switch", Image=1,
                            Used=1).Create()
        if (5 not in Devices):
            Domoticz.Device(Name=str(Parameters["Mode3"]) + ' - plug4', Unit=5, TypeName="Switch", Image=1,
                            Used=1).Create()
...
def onCommand(Unit, Command, Level, Hue):
    ...
    Domoticz.Log("onCommand called for Unit " + str(Unit) + ": Parameter '" + str(Command) + "', Level: " + str(
        Level) + " , Connected : " + str(isConnected))
...
def save():
    ...
    if os.path.exists(path):
        ...
        try:
            ...
            return False
    try:
        Domoticz.Device(Name=str(Parameters["HardwareID"]) + "-" + str(numberDev) + " " + custom, Unit=numberDev,
                        TypeName="Selector Switch", Type=244, Switchtype=9, Subtype=73).Create()
    except:
    ...
    try:
        if (Parameters["Mode3"] == 'RM2T' or Parameters["Mode3"] == 'RM2'):
            device = broadlink.rm(host=(Parameters["Address"], 80), mac=bytearray.fromhex(Parameters["Mode1"]),
                                  devtype=Parameters["Mode3"])
        elif (Parameters["Mode3"] == 'A1'):
            device = broadlink.a1(host=(Parameters["Address"], 80), mac=bytearray.fromhex(Parameters["Mode1"]),
                                  devtype=Parameters["Mode3"])
        elif (Parameters["Mode3"] == 'SP1'):
            device = broadlink.sp1(host=(Parameters["Address"], 80), mac=bytearray.fromhex(Parameters["Mode1"]),
                                   devtype=Parameters["Mode3"])
...
def createIniImport():
    ...
    for i in range(0, len(data_code)):
        ...
        result = "Numrec : " + str(i) + " Button number: " + str(button) + " " + "Number name : " + str(
            numName) + " Name : " + name + " " + buttonName + " Code : " + str(code)
    ...
    Domoticz.Log(
        'Simulate.txt file has been created with all codes on it. Click <a target="_blank"  href="' + filelink + '" style="color:blue">here</a> to see the path')
